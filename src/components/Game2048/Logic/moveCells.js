...
const moveCells = (initCells, direction) => {
  ...
  const matrix = Array.from(new Array(4), () => Array.from(new Array(4),() => 0),) // first argumet - 4 arrays from new Array() with undefained values, second arg. - function with another 4 arr.
  cells.forEach(cell => {
  ...
  Array
    ...
    .forEach((colNum) => {
      ...
      printString += Array
        .from(new Array(4), (x, i) => i)
        .map(rowNum =>
        JSON.stringify(matrix[colNum][rowNum]).padStart(40, ' ')
        )
        .join(', ')
      printString += ',\n'
    })
